...
dashboardRouter.get('/stats', async (req, res) => {
    ...
    const [empCount, leaveCount] = await Promise.all([
        query(`SELECT COUNT(*)::text AS count FROM employees e WHERE ${empFilter.sql}`, empFilter.params),
        query(`SELECT COUNT(*)::text AS count FROM leaverequests lr
       WHERE ${leaveFilter.sql} AND lr.status = 'pending'`, leaveFilter.params),
    ]);
    res.json({
